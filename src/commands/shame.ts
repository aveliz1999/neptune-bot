...
import {GameType, getGameFromId, getGamesForPlayer} from "../db";
    ...
    async run(message: CommandoMessage, args: {}, fromPattern: boolean, result?: ArgumentCollectorResult): Promise<any> | null {
        ...
        if(games.length > 1) {
            ...
            for(let r of reactions.array()) {
                ...
                await this.shameGame(game, message);
            }
        }
        else {
            return this.shameGame(games[0], message);
        }
    }

    async shameGame(game: GameType, message: CommandoMessage) {
